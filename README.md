...
- 机构详情中，未设置环境组的历史环境会显示在 `デフォルト` 组。管理员可在机构标题旁直接新增组、调整组显示顺序、把环境移动到指定组、将整个组移动到其他机构，并删除空组；每个机构至少保留一个组。迁移到只有空 `デフォルト` 组的新机构时，系统会自动删除该空默认组。机构摘要的收缩态会按环境组画成容器，方便查看人员快速识别。
