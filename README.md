...
- 生产环境页签复用环境检索的同一套组织、环境组、环境卡片和保存逻辑，只显示 `用途=生産` 的记录。新增生产记录时固定写入 `環境種別=本番`、`用途=生産`。旧 `production.csv` 中尚未迁移的现场本番数据会在读取 `portal_data.jsp` 时只读合并为统一环境记录和服务器信息，避免旧本番记录在合并页签后不可见；管理员后续编辑保存时再写入新的统一格式。
