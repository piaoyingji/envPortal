...
- 客户主档支持按客户编码或名称过滤，适合导入大量客户主档后的快速定位。
...
- 客户环境页采用桌面工作台最小宽度与移动查询布局：桌面保持右侧数据导航，窄屏手机/平板切换为单列查询体验。
- 环境健康检查会返回 HTTP 状态、响应时间、TTL 和 OS 推测，并按分钟刷新。
- 数据库信息支持地址、端口、实例/库、用户、密码、类型、版本字段。
- 数据库类型和版本支持自动探测，当前支持 Oracle 与 PostgreSQL。
- 远程连接信息支持多条 RDS/RDP/SSH 记录。RDS/RDP 可一键启动 mstsc，并自动把密码复制到剪贴板。
- 远程连接可标记为私用或共用。共用 RDS/RDP 通过系统菜单中的公共远程连接主档维护，只有明确共用的 `host + port` 才可跨客户/环境复用。
- 远程连接按钮按 OneCRM 服务端 TCP 检测结果启用；检测不通时会显示无法连接，避免把需要 VPN 或网络放通的目标误认为可直连。
