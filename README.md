...
- 环境卡片只维护该环境自身的 URL、登录信息、DB、服务器信息和 tags，组织/组信息不再混在环境编辑里。服务器信息支持多条维护，每条可编辑 `サーバ名`、连接类型、用户、密码和连接地址；`サーバ名` 可用于 AP、DB、AP/DB 共用、踏み台或自定义名称。`rdp.csv` 中的 `構築環境名` 用于把多台服务器显式绑定到同一环境，旧数据缺失该列时继续按 URL/DB 主机匹配兼容读取。
- 环境组移动使用环境卡片标题区的独立图标按钮，选择目标组并确认后立即保存并重新渲染机构画面。
- 每个服务器/环境支持独立业务标签，标签可以跨机构自由过滤，例如 `DEMO`、`教育`、`OneHR`。环境区分和用途改为枚举字段，不再作为普通标签显示。
- 标签过滤支持分类展示、多选 AND 条件，分类由系统管理中的 TAG 分类管理画面显式维护；系统自动生成标签默认进入“其他”。Clear 只清除用户手工选择的 TAG 条件，角色 TAG 数据权限仍作为基础数据范围生效，右侧画面回到普通组织摘要布局。管理员在全局 TAG 过滤结果中仍可使用机构编辑、环境追加和组追加。TAG 分类保存会保留 TAG 显示设定，并通知已打开的首页重新读取分类和配色后刷新渲染。
...
- 组织选择支持五十音页签切换。五十音索引使用链接式换行布局，较长标签会作为整体换行显示。`org_readings.js` 不存在时，后端会通过 `/org_readings_status.jsp` 自动补齐并生成本地读音映射。
- 生产环境页签复用环境检索的同一套组织、环境组、环境卡片和保存逻辑，本质上是对环境检索应用 `用途=生産` 的大过滤。拥有本番查看权限的角色，在环境检索页也能看到其可见范围内的本番服务器；本番页签只保留有本番环境卡片的机构，空机构仍可在环境检索页存在和维护。新增生产记录时固定写入 `環境種別=本番`、`用途=生産`。旧 `production.csv` 中尚未迁移的现场本番数据会在读取 `portal_data.jsp` 时只读合并为统一环境记录和服务器信息，避免旧本番记录在合并页签后不可见；管理员后续编辑保存时再写入新的统一格式。
- 首页按机构显示紧凑摘要，环境卡片通过显式展开/收回按钮查看详情，减少默认页面空白。展开后的环境卡片可直接进入编辑态；编辑态收回时会自动退出编辑态，存在未保存修改时先提示是否保存。TAG 过滤结果也保持摘要态，只在用户点击展开时显示详情。
...
角色数据权限使用 `dataTags` 数组保存。管理员角色不限制数据，其他角色只显示命中 `dataTags` 中任一有效业务 TAG 的环境。当非管理员角色的有效 `dataTags` 覆盖当前全部有效 TAG 时，运行时视为全量数据范围，只保留功能权限限制。有效 TAG 只来自 `tags.json` 中手工维护的业务 TAG；如果某个 TAG 已经从数据中删除，即使仍残留在角色配置中，也不会继续授予可见数据。为避免旧角色配置在升级瞬间失效，旧数据中残留的 `社内`、`本番`、`受入`、`テスト`、`開発` 等结构性 TAG 仍可用于既有角色匹配，但首页 TAG 过滤、编辑框和显示皮肤不再把它们作为普通 TAG 展示。
...
`2.6.4` 修复生产环境页签合并后的旧数据兼容问题。后端读取 `portal_data.jsp` 时会把未迁移的旧 `production.csv` 记录映射为统一环境记录，并按 `canViewProduction` 权限过滤；现场旧文件本身不会被页面读取改写。该版本属于 PATCH 递进。

`2.6.5` 明确生产环境页签是环境检索的子集。环境检索入口对拥有本番查看权限的角色可见，本番页签只按 `用途=生産` 展示非空机构，环境检索页继续允许空机构存在。该版本属于 PATCH 递进。
...
`2.6.10` 修复全局 TAG 过滤结果页的管理员操作区。TAG 过滤只收窄显示数据，不改变管理员在机构标题上的编辑、环境追加和组追加入口。该版本属于 PATCH 递进。

`2.6.11` 调整组织选择五十音索引显示。索引项改为链接式换行布局，较长标签按整体换行，避免窄侧栏中出现截断和横向滚动。该版本属于 PATCH 递进。
